...
ALTER TABLE actor DELETE COLUMN middle_name;

-- 4a. List the last names of actors, as well as how many actors have that last name.
SELECT last_name, COUNT(*) AS 'count_last_name' FROM actor GROUP BY last_name;
...
SELECT s.staff_id, s.first_name, s.last_name, SUM(p.amount) AS 'Total_Amt_Rung'
FROM staff as s
JOIN payment as p 
USING (staff_id)
...
SELECT title, COUNT(actor_id) AS '#_of_Actors'
FROM film
...
SELECT a.store_id, SUM(a.amount) AS 'revenue'
FROM (
...
CREATE VIEW TOP_5_GENRES_BY_GROSS_REVENUE
AS (
	SELECT D.category_name, SUM(C.amount) AS 'gross_revenue'
	FROM (
